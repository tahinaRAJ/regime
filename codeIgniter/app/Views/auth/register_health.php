<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Informations de santé</title>
    <link rel="stylesheet" href="<?= base_url('assets/css/style.css') ?>">
</head>
<body>
    <div class="nature-background">
        <div class="floating-leaf leaf-1"></div>
        <div class="floating-leaf leaf-2"></div>
        <div class="floating-leaf leaf-3"></div>
        <div class="floating-leaf leaf-4"></div>
    </div>

    <div class="login-container">
        <div class="wellness-card">
            <div class="organic-border"></div>

            <div class="mindful-header">
                <div class="zen-logo">
                    <svg width="56" height="56" viewBox="0 0 56 56" fill="none">
                        <circle cx="28" cy="28" r="26" stroke="currentColor" stroke-width="2" fill="none" opacity="0.6"/>
                        <circle cx="28" cy="28" r="18" stroke="currentColor" stroke-width="1.5" fill="none" opacity="0.8"/>
                        <circle cx="28" cy="28" r="10" fill="currentColor" opacity="0.9"/>
                        <path d="M28 18v20M18 28h20" stroke="white" stroke-width="1.5" stroke-linecap="round"/>
                    </svg>
                    <div class="zen-glow"></div>
                </div>
                <h1>MonNouveauMoi</h1>
                <p>Étape 2/2 — Taille & poids</p>
            </div>

            <div style="margin-bottom: 14px; opacity: 0.9;">
                <div><strong>Nom:</strong> <?= esc($registerUser['name'] ?? '') ?></div>
                <div><strong>Email:</strong> <?= esc($registerUser['email'] ?? '') ?></div>
                <div><strong>Genre:</strong> <?= esc($registerUser['genre'] ?? '') ?></div>
                <div><strong>Âge:</strong> <?= esc($registerUser['age'] ?? '') ?></div>
            </div>

            <?php if (!empty($erreur)) : ?>
                <div class="gentle-error" style="display:block;margin-bottom:12px;">
                    <?= esc($erreur) ?>
                </div>
            <?php endif; ?>

            <div class="gentle-error" id="clientError" style="display:none;margin-bottom:12px;"></div>

            <form class="harmony-form" method="post" action="/register/health" novalidate>
                <?= csrf_field() ?>

                <div class="organic-field">
                    <div class="field-nature"></div>
                    <input type="number" step="0.01" id="height" name="height" required value="<?= esc($data['height'] ?? '') ?>">
                    <label for="height">Taille (cm)</label>
                </div>

                <div class="organic-field">
                    <div class="field-nature"></div>
                    <input type="number" step="0.01" id="weight" name="weight" required value="<?= esc($data['weight'] ?? '') ?>">
                    <label for="weight">Poids (kg)</label>
                </div>

                <button type="submit" class="harmony-button">
                    <div class="button-earth"></div>
                    <span class="button-text">Créer le compte</span>
                    <div class="button-growth">
                        <div class="growing-circle circle-1"></div>
                        <div class="growing-circle circle-2"></div>
                        <div class="growing-circle circle-3"></div>
                    </div>
                    <div class="button-aura"></div>
                </button>
            </form>

            <div class="nurture-signup" style="margin-top: 18px;">
                <a href="/register" class="growth-link">Retour à l étape 1</a>
            </div>
        </div>
    </div>

    <script>
        (function () {
            const form = document.querySelector('form.harmony-form');
            const errorBox = document.getElementById('clientError');

            if (!form || !errorBox) return;

            function showError(message) {
                errorBox.textContent = message;
                errorBox.style.display = 'block';
            }

            function clearError() {
                errorBox.textContent = '';
                errorBox.style.display = 'none';
            }

            form.addEventListener('submit', function (e) {
                clearError();

                const heightValue = (document.getElementById('height')?.value || '').trim();
                const weightValue = (document.getElementById('weight')?.value || '').trim();

                if (!heightValue || !weightValue) {
                    e.preventDefault();
                    showError('Veuillez remplir tous les champs');
                    return;
                }

                const height = Number(heightValue);
                const weight = Number(weightValue);
                if (!Number.isFinite(height) || height <= 0 || !Number.isFinite(weight) || weight <= 0) {
                    e.preventDefault();
                    showError('Taille ou poids invalide');
                    return;
                }
            });
        })();
    </script>
</body>
</html>
